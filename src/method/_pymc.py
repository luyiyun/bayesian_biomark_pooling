...
def _set_prior_for_hierachical_model(
    n_studies: int,
    prior_betax: str,
    prior_sigma_ws: str,
    prior_a_std: float,
    prior_b_std: float,
    prior_beta0_std: float,
    prior_sigma_ab0: str,
) -> Tuple[pm.Distribution]:
    if prior_betax == "flat":
        betax = pm.Flat("betax")
    elif prior_betax == "normal":
    ...
    a = pm.Normal("a", 0, prior_a_std)
    b = pm.Normal("b", 0, prior_b_std)
    beta0 = pm.Normal("beta0", 0, prior_beta0_std)
    # beta0 = pm.Flat("beta0")
...
class HierachicalModel(SimpleModel):
    ...
    def _set_prior(
        ...
        return _set_prior_for_hierachical_model(
            n_studies,
            self._prior_betax,
            self._prior_sigma_ws,
            self._prior_a_std,
            self._prior_b_std,
            self._prior_beta0_std,
            self._prior_sigma_ab0,
        )


class HierachicalModel_WX(PymcModel):
    """建模方向改为w -> x -> y，这样就减少了一个建模设置，即x的建模"""
        ...
        with pm.Model() as model:
            ...
            for i, WY in enumerate(WY_xUnKnow):
                ...
                X_no_obs_i = pm.Normal(
                    "X_no_obs_%d" % i, a_s[i] + b_s[i] * W, sigma_ws[i]
                )
                pm.Bernoulli(
                    "Y_%d_no_obs_X" % i,
                    logit_p=beta0s[i] + betax * X_no_obs_i,
